<?php
    ...
    if ( isset( $jws_option['quo_info_form_status'] ) && ! $jws_option['quo_info_form_status'] ) {
    	return;
    }
    $id = get_the_ID();
    if( isset($_GET['id']) ) {
...
?>
    ...
    <h3><?php echo esc_html__('Get a Quote','idealauto'); ?></h3>  
   <div class="car-info">
	...
	<div id="quote-with-dealer">
                    <h5 class="per-title"><?php echo esc_html__( 'PERSONAL INFORMATION', 'idealauto' ); ?></h5>
					<?php
					if ( isset( $jws_option['quo_info_form'] ) && ! empty( $jws_option['quo_info_form'] ) && '1' === (string) $jws_option['quo_info_contact_7'] ) {
						echo do_shortcode( $jws_option['quo_info_form'] );
					} else {
						?>
						<form  method="post" id="quote-form-<?php echo esc_attr($id); ?>">
							<div class="row">
								<input type="hidden" name="action" class="form-control" value="car_quote_action">
								<input type="hidden" name="car_id" value="<?php echo esc_attr($id); ?>">
								<?php wp_nonce_field( 'quo-nonce', 'quo-nonce' ); ?>
								<div class="col-xl-6">
									<div class="form-group">
										<label><?php echo esc_html__( 'First Name', 'idealauto' ); ?>*</label>
										<input type="text" name="first_name" class="form-control jws_validate" quouired maxlength="25">
									</div>
								</div>
								<div class="col-xl-6">
									<div class="form-group">
										<label><?php echo esc_html__( 'Last Name', 'idealauto' ); ?>*</label>
										<input type="text" name="last_name" class="form-control jws_validate" quouired maxlength="25">
									</div>
								</div>
                                <div class="col-xl-12">
                                    	<div class="form-group">
					           ...
					           <div class="col-xl-6">
									<div class="form-group">
										<div class="jws-recapcha" id="recap3-<?php echo esc_attr($id); ?>"></div>
									</div>
								</div>
								...
								<div class="col-xl-6">
									<div class="form-group btn-form">
										...
										<button id="submit_quouest" class="btn-main elementor-button" ><span><?php echo esc_html__( 'Submit', 'idealauto' ); ?></span></button>
										<div class="quote-msg" style="display:none;"></div>
									</div>
								</div>
							</div>
						</form>
